...
**Date:** April 22, 2026
**Team:** Brandon Werner
**Status:** v1 released. Three auth modes working (Agent User / Delegated / Bot Gateway). Progressive identity state machine. **~618 tests** across the suite. MCP tools + 4 background tasks (Teams 5s / email 60s / chat-discovery 120s / daily summary 5pm PDT). Multi-tenant lightweight chat shipped. **Mind-body split complete** — body-first prompt architecture loads locally, persona-sati MCP wired for personality/memory when configured. ADR-005 cloud-memory Phases 1, 2, 5, 6a shipped; blob-hosted operational storage is opt-in via `setup.sh --cloud-memory`. Efferent-copy middleware shipped and immediately hot-fixed for self-spawn cascade (PRs #35/#36). Leader/slave gating ripped out per "one stdio client per process" reality. **Open issue:** Claude Code 2.1.117 silently dropping `notifications/claude/channel` renders despite entraclaw pushing correctly — see Learning #39.

---

## What's New Apr 22 (efferent-copy + leader-gate rip + Claude Code channel regression)

Three PRs merged in sequence; one deep debugging session; one open-issue write-up documenting behavior that looks like a Claude Code 2.1.117 regression.
...
**Open issue — Claude Code 2.1.117 drops `notifications/claude/channel` renders.** Despite entraclaw correctly pushing (write_stream.send completes, log line fires), the notification does not surface in the LLM's turn context. Verified by grepping the active session's `~/.claude/projects/<slug>/<session>.jsonl` for the test DM content — the only hits are our own `read_teams_messages` tool results and echoes. Zero Claude-Code-injected channel entries. The server's end is indistinguishable from working; the client-side gate `r1_`/`hO_` (name-mangled per release) is returning `action:"skip"` for one of 5 reasons (`capability|disabled|auth|policy|session|allowlist`), and Claude Code doesn't expose which. `/login` re-auth did not resolve it. Gate function body is byte-identical between 2.1.114 (last confirmed working) and 2.1.117 (current) — the regression is elsewhere in the registration or rendering path, not in the gate. **See Learning #39 for full diagnostic evidence and next-step suggestions.**
