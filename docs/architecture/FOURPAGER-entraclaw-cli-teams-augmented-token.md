...
- **Multiple Microsoft product groups need it.** GitHub Copilot CLI (Patrick), ClawPilot (Dave Fixture), the Maya POC team, Entraclaw research, and Aashima's digital teammate work are all converging on the same need. Without a Teams-side answer, every team invents its own string-prefix workaround that does not actually solve attribution. With a Teams-side answer, all of them get a consistent customer experience.
