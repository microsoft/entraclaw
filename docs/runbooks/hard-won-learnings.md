...
### Learning #46: Entraclaw MCP Dies After Minutes of Sustained Activity — Root Cause Not Yet Isolated

**Date:** 2026-04-24
**Status:** **OPEN — amplifiers fixed, underlying symptom reduced but not eliminated. See `docs/runbooks/mcp-disconnect-investigation.md` for the full debug dossier.**
**Context:** Entraclaw MCP (stdio child of Claude Code) becomes progressively slower then disconnects, typically within 2–10 minutes of sustained activity (inbound Teams DMs, email poll cycles, daily-summary scheduler). UX: first 1–3 tool calls return in <1s; later ones stall 5–60s; `/mcp` shows `disconnected`; a manual `Reconnect entraclaw` respawns the child and the cycle repeats.
**Problem:** Child process is **reaped, not crashed** — zero Python tracebacks on the parent MCP process at the drop, `~/.entraclaw/logs/entraclaw.log` simply stops at the cutoff. Parent Claude CLI sustained at 27–83% CPU and ~800 MB–1 GB RSS while entraclaw is active. Shape is classic "consumer stopped draining, producer eventually killed" — not "producer crashed."
**Amplifiers found and fixed (but symptom not gone):**
1. **PR #40 (`9c74cd1`) — `logger.propagate = False` on `entraclaw` logger.** Root cause: FastMCP's `configure_logging()` installs a `RichHandler` on the root logger via `basicConfig`; every entraclaw record rendered twice on stderr (JSON from our StreamHandler + rich from root). Halved the entraclaw-originated stderr volume the parent CLI had to drain. Does NOT silence `httpx.*` / `msal.*` propagation (still open).
2. **PR #41 (`fc2e49b`) — wrapper-marker self-ref detection.** Root cause: debug wrapper `scripts/entraclaw-mcp-debug.sh` named in `.mcp.json`'s `command` bypassed `_is_self_referential_peer` (see Learning #45) and retriggered the self-spawn cascade from Learning #37 at 1-child-deep. Symptom was twin-spawn: two full boots per reconnect, 2× token flows, 2× persona-sati fetches, 2× Graph calls. Wrapper now carries `# entraclaw-self-ref-target: <path>` marker; check reads up to 16 KB of the script and matches declared target against `sys.argv[0]` / `sys.executable`.
3. **Removed 4 slow throttling tests** (`TestCreateOrFindChat::test_rate_limited`, `TestTeamsSend::test_rate_limited`, `TestUpdatePlaceholder::test_rate_limited_raises`, `TestDeleteChatMessage::test_raises_rate_limit_on_429`) — 357s of real `time.sleep()` through `Retry-After`. Unrelated to runtime disconnect; removed because they were pushing the test suite past Bash 300s timeout and masking other signal.

**Leading hypothesis (not confirmed):** Parent Claude Code CLI stdio-drain backpressure. When entraclaw emits bursts during a Teams push or email poll, the CLI's render path (SSE to API, markdown repaint, tool-schema validation) saturates — CPU climbs to 80%+, stdout pipe from the child stops being drained, after some threshold the CLI reaps the child. Evidence: zero child tracebacks, sustained parent CPU, "fast at first, then slow, then gone" shape. Against: we have no direct read-side measurement on the parent.

**Secondary hypothesis:** Blob-write-on-Teams-push hot path. Every inbound Teams push synchronously does token check + blob GET + blob PUT with ETag concurrency (3–5 HTTPS round-trips) before returning. Under bursts these serialize and can pin an event-loop task, which delays the `notifications/claude/channel` push; parent CLI may time out waiting. To test: add `perf_counter` spans around the blob call and log slow ones before attempting the full async-queue decouple.

**What to try next (in order, see runbook for full detail):** (1) Flip `.mcp.json` to the self-ref-marker wrapper, capture `/tmp/entraclaw-debug.log`, measure stderr rate and whether drops correlate with event types. (2) Set `propagate=False` / WARNING on `httpx` and `msal` loggers too. (3) Add `perf_counter` around blob I/O on the push path; if it's hot, decouple via asyncio queue. (4) Run entraclaw under SSE transport instead of stdio; if drops disappear, CLI backpressure is confirmed. (5) `py-spy dump` the parent CLI mid-drop. (6) File upstream if SSE is clean and stdio is not.

**What was ruled out (do NOT re-investigate from zero):** efferent-copy cascade (fixed PR #36 / PR #41), leader/slave gate (ripped out PR #36 — see Learning #38), dev-channel launch flag (Learning #39 — current process has the correct double-dash form), Learning #36 venv corruption (current `config.__file__` resolves to parent src tree), Learning #44 parent-rename orphan (`pyvenv.cfg` and shebangs clean).

**Prevention:** (1) Any agent picking this up MUST read `docs/runbooks/mcp-disconnect-investigation.md` end-to-end first. Do not restart the debug from scratch. (2) Ship any new diagnostics (perf spans, queue decouple) behind an env flag so they can be toggled in production without re-plumbing `.mcp.json`. (3) When silencing third-party loggers, do NOT revert `entraclaw`'s `propagate=False` — they are independent fixes. (4) Keep the debug wrapper **opt-in** via `.mcp.json` flip; do not make it default on `main` (it writes to `/tmp/entraclaw-debug.log`, which is a single-user convention and will collide under multi-user or containerized setups).

**Evidence/references:** `docs/runbooks/mcp-disconnect-investigation.md` (the living runbook); commits `9c74cd1` (PR #40) and `fc2e49b` (PR #41); `src/entraclaw/logging_config.py` (propagate fix); `src/entraclaw/efferent_copy.py::_is_self_referential_peer` (wrapper-marker check); Learnings #36, #37, #38, #39, #44, #45 (all the "ruled out" regressions).
