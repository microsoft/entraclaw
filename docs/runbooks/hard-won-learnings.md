...
### Learning #39: Claude Code 2.1.117 Silently Drops `notifications/claude/channel` Renders

**Date:** 2026-04-22
**Status:** **OPEN — unresolved as of writeup.**
**Context:** After PR #36 fixed the server-side cascade and ripped the leader gate, entraclaw's end was verifiably correct — the push fires, `write_stream.send(session_message)` completes, and `Pushed Teams message from Brandon Werner: <content>` logs. Brandon still did not see inbound DMs surface in the LLM's turn context in Claude Code 2.1.117.
**Problem:** No `notifications/claude/channel` entries in the active session's transcript (`~/.claude/projects/<slug>/<session>.jsonl`). The only hits for the test DM content ("Hi Hi Hi") are our own `read_teams_messages` tool results and echoes. Zero Claude-Code-injected channel entries. The server's end is indistinguishable from working; the client isn't rendering.
**Investigation done:**
...
7. The CLI is invoked with `claude -dangerously-load-development-channels server:entraclaw --resume <id>`, which is the intended "local dev" allowlist escape hatch.
**Suspected root cause:** Regression in Claude Code 2.1.117 elsewhere in the channel registration/rendering pipeline (the gate function itself is unchanged). Possibilities: (a) the `-dangerously-load-development-channels` flag parsing changed; (b) the list `fY()`/`Nw()` returns isn't populated the same way; (c) the handler that routes the notification to the LLM turn context was refactored and regressed. Worked on **2.1.114 (installed Apr 19)**; confirmed-broken on **2.1.117 (installed Apr 21 17:50)**. Versions **2.1.116 and 2.1.117 bracket the regression**.
**Workaround (none shipped yet):** Three paths discussed:
1. Accept and work around — have the LLM explicitly `read_teams_messages` when the human pings in terminal. Loses "push" magic, 100% reliable.
2. Downgrade test — manually point `~/.claude-cli/CurrentVersion` to `2.1.114` and confirm. If channels render again, the regression is confirmed and the fix is "don't upgrade past 2.1.x" + file a Claude Code issue.
3. Replace channel-push with a `UserPromptSubmit` hook that reads `~/.entraclaw/data/interactions/<today>.jsonl` and injects any inbound-DMs-since-last-turn as `additionalContext`. Fully under our control, survives Claude Code upgrades.
**Evidence/references:** `docs/engineering-status.md` "What's New Apr 22" section; `~/.entraclaw/logs/entraclaw.log` timestamps showing successful `Pushed Teams message from ...` lines; current session's `~/.claude/projects/-Volumes-Development-HD-openclaw-identity-research/<session>.jsonl` showing zero Claude-Code-injected channel entries.
